...
/**
 * 센서로 동작되는 윗몸일으키기 대한 내용을 관리하는 클래스
 * 
...
SurfaceHolder.Callback {
	...
	int exercise_id = 3; // 운동 번호
	int cnt_ding; // count소리

	boolean checkin = false; // 근접센서가 동작되면, check in 나가면 check out
	boolean start_flag = false; // 센서 스타트 flag
	boolean pausing_flag = true; // 일시정지 flag

	Button btn_start; // 스타트 버튼
	ImageButton btn_toggle_play; // 동영상 재생/일시정지 토글 버튼

	TextView text_cnt = null; // 센서 카운트

	int counter = 20; // 횟수 카운트
	...
	public void CheckCount() {
		if (checkin == true) {
			...
			// 사운드 출력
			try {
				Thread.sleep(100); // sensor와 sound플레이간 간격 조절
			} catch (Exception ee) {
			}
